...
const Person = require('../models/Buyer');
...
router.post('/likeProperty', jwtAuthMiddleware, async (req, res) => {
    try {
        const { propertyId } = req.body;
        const buyerId = req.user.id;

        const buyer = await Person.findById(buyerId);

        if (buyer.likedProperties.includes(propertyId)) {
            return res.status(400).json({ message: "Property already liked" });
        }

        buyer.likedProperties.push(propertyId);
        await buyer.save();
...
router.delete('/unlikeProperty/:propertyId', jwtAuthMiddleware, async (req, res) => {
    try {
        ...
        const index = buyer.likedProperties.indexOf(propertyId);
        if (index === -1) {
            return res.status(400).json({ message: "Property not liked" });
        }

        // Remove the property ID from the list of liked properties
        buyer.likedProperties.splice(index, 1);
        await buyer.save();
